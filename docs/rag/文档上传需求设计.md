...
| 模块 | 职责 |
|------|------|
| `knowledge-web` | 新增「知识管理 → 资料上传」独立页面；通过代理访问 `knowledge-rag` 接口 |
| `knowledge-rag` | 文件上传、MinerU 解析调度、TXT 转 Markdown 兜底转换、文档落库、版本管理 |
...
| 表名 | 归属项目 | 说明 |
|------|---------|------|
| `knowledge_document` | `knowledge-rag` | 文档主表，解析/转换成功后生成 |
| `knowledge_upload_document_record` | `knowledge-rag` | 文档上传记录表 |
| `knowledge_mineru_parse_task` | `knowledge-rag` | MinerU 解析任务表 |
| `knowledge_mineru_parse_detail_task` | `knowledge-rag` | MinerU 解析批次/分段明细表 |
