...
```mermaid
graph TD
    Start["lifespan 入口"] --> Redis["创建 Redis 连接池<br/>RedisUtil.create_redis_pool"]
    ...
    DB --> RedisCheck["检查 Redis 连接<br/>RedisUtil.check_redis_connection"]
    RedisCheck --> Dict["缓存字典表<br/>RedisUtil.init_sys_dict"]
    Dict --> Config["缓存参数配置<br/>RedisUtil.init_sys_config"]
...
graph TD
    ...
    LockCancel --> RedisClose["关闭 Redis 连接池<br/>RedisUtil.close_redis_pool"]
