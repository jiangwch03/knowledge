...
sequenceDiagram
    ...
    Req->>MW: 请求进入
    MW->>MW: get_current_session() 预检
    alt 已有 session
        MW->>Handler: 直接放行，不创建不清理
    else 无 session
...
**作用**：使 `get_current_session()` 在非 `@transactional` 方法中也能返回当前请求的 session（不自动 commit，请求结束后 session 关闭）。若进入时已有 session（如子应用嵌套），则直接复用，不重复创建。
...
> 所有 session 注入入口均内置预检：调用 `get_current_session()` 检查是否已有可用 session，有则直接复用，无才创建新 session。避免重复创建连接池连接，同时兼容嵌套场景。
