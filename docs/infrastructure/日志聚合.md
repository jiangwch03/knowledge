...
graph LR
    ...
    Rag["knowledge-rag"] -->|produce| T3["log:operation:knowledge-rag"]
    Rag -->|produce| T4["log:login:knowledge-rag"]
    T1 --> C1["@consumer<br/>group: log_writer:knowledge-admin"]
    T2 --> C1
    T3 --> C2["@consumer<br/>group: log_writer:knowledge-rag"]
    T4 --> C2
    C1 --> DB1["admin 端落库"]
    C2 --> DB2["rag 端落库"]
```
