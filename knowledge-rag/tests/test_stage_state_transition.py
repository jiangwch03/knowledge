...
from knowledge_rag.enums.document_upload_status_enum import DocumentUploadStatus
from knowledge_rag.enums.mineru_parse_detail_state_enum import MineruParseDetailState
from knowledge_rag.enums.mineru_parse_task_status_enum import MineruParseTaskStatus
from knowledge_rag.mapper.do.parse_detail_task_do import KnowledgeMineruParseDetailTask
from knowledge_rag.mapper.do.parse_task_do import KnowledgeMineruParseTask
from knowledge_rag.mapper.do.upload_record_do import KnowledgeUploadDocumentRecord
from knowledge_rag.service.document_upload_parse_service import DocumentUploadParseService
...
def _patch_transactional():
    ...
    import knowledge_rag.service.document_upload_parse_service as svc_module
...
class TestStage2PathARetryLinkFailed:
    ...
    async def test_link_failed_retry_success(self):
        ...
        with (
            patch(
                'knowledge_rag.service.document_upload_parse_service.KnowledgeMineruParseTaskDao.get_task_by_id',
                new_callable=AsyncMock,
                return_value=mock_task,
            ),
            patch(
                'knowledge_rag.service.document_upload_parse_service.KnowledgeUploadRecordDao.get_record_by_id',
                new_callable=AsyncMock,
                return_value=mock_record,
            ),
            patch(
                'knowledge_rag.service.document_upload_parse_service.KnowledgeMineruParseDetailTaskDao.get_details_by_task_id',
                new_callable=AsyncMock,
                return_value=[],
            ),
            patch.object(
            ...
            patch(
                'knowledge_rag.service.document_upload_parse_service.KnowledgeMineruParseTaskDao.update_status',
                new_callable=AsyncMock,
            ) as mock_update_task,
            patch(
                'knowledge_rag.service.document_upload_parse_service.KnowledgeUploadRecordDao.update_status',
                new_callable=AsyncMock,
            ) as mock_update_record,
            patch(
                'knowledge_rag.service.document_upload_parse_service.KnowledgeMineruParseDetailTaskDao.batch_add_details',
                new_callable=AsyncMock,
            ) as mock_add_details,
            patch.object(
                DocumentUploadParseService,
            ...
            patch(
                'knowledge_rag.service.document_upload_parse_service.KnowledgeMineruParseDetailTaskDao.update_detail',
                new_callable=AsyncMock,
            ),
        ):
...
class TestStage2PathBUploadFailed:
    ...
    async def test_upload_failed_timeout_converge(self):
        ...
        with (
            patch(
                'knowledge_rag.service.document_upload_parse_service.KnowledgeMineruParseDetailTaskDao.batch_update_state',
                new_callable=AsyncMock,
...
class TestStage3PollResults:
    ...
    async def test_all_details_parsed_success(self):
        ...
        with (
            patch(
                'knowledge_rag.service.document_upload_parse_service.MineUClient'
            ) as mock_client_cls,
            patch(
                'knowledge_rag.service.document_upload_parse_service.KnowledgeMineruParseDetailTaskDao.get_details_by_task_id',
                new_callable=AsyncMock,
...
class TestStage4MarkdownMerge:
    ...
    async def test_md_merge_success(self):
        ...
        with (
            ...
            patch(
                'knowledge_rag.service.document_upload_parse_service.KnowledgeMineruParseTaskDao.get_active_task_by_record_id',
                new_callable=AsyncMock,
                return_value=None,
            ),
            patch(
                'knowledge_rag.service.document_upload_parse_service.KnowledgeMineruParseTaskDao.get_tasks_by_record_id_and_status',
                new_callable=AsyncMock,
                return_value=[MagicMock(parse_task_id=50)],
            ),
            patch(
                'knowledge_rag.service.document_upload_parse_service.KnowledgeMineruParseDetailTaskDao.get_details_by_task_ids',
                new_callable=AsyncMock,
                return_value=[
                    MagicMock(
                        detail_id=1,
            ...
            patch(
                'knowledge_rag.service.document_upload_parse_service.MineruZipMergeService.download_and_extract_details',
                new_callable=AsyncMock,
                return_value=mock_merge_result,
            ),
            patch(
                'knowledge_rag.service.document_upload_parse_service.KnowledgeMinioService.upload_stream',
                new_callable=AsyncMock,
            ),
            patch(
                'knowledge_rag.service.document_upload_parse_service.KnowledgeMinioService.get_object_url',
                return_value='https://minio/final.md',
            ),
            patch(
                'knowledge_rag.service.document_upload_parse_service.KnowledgeDocumentDao.update_latest_by_title',
                new_callable=AsyncMock,
            ),
            patch(
                'knowledge_rag.service.document_upload_parse_service.KnowledgeDocumentDao.get_max_version_by_title',
                new_callable=AsyncMock,
                return_value=None,
            ),
            patch(
                'knowledge_rag.service.document_upload_parse_service.KnowledgeDocumentDao.add_document',
                new_callable=AsyncMock,
            ),
            patch(
                'knowledge_rag.service.document_upload_parse_service.KnowledgeUploadRecordDao.update_status',
