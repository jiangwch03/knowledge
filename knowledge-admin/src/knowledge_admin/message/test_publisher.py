...
class AdminMessageTestPublisher:
    """
    admin 端测试发送类(对应 consumer/test_consumer.py)
    ...
    async def send_demo(cls, payload: dict | None = None) -> str | None:
        ...
        try:
            ...
            msg_id = await MessageStreamService.produce(
                topic=cls.TOPIC,
                value=value,
                key='admin-startup-demo',
            )
            logger.info(
                f'📤 [admin-test-publisher] 已发送测试消息: '
                f'topic={cls.TOPIC} id={msg_id} value={value}'
            )
            return msg_id
        except MessageStreamError as e:
            logger.warning(
                f'⚠️ [admin-test-publisher] 测试消息发送失败(忽略,不影响启动): {e}'
            )
            return None


__all__ = ['AdminMessageTestPublisher']
