...
class DeptService:
    ...
    async def get_dept_tree_services(
        ...
        :return: 部门树信息对象
        """
        dept_list_result = await DeptDao.get_dept_list_for_tree(page_object, data_scope_sql)
    ...
    async def get_dept_for_edit_option_services(
        ...
        :return: 部门树信息对象
        """
        dept_list_result = await DeptDao.get_dept_info_for_edit_option(page_object, data_scope_sql)
    ...
    async def get_dept_list_services(
        ...
        :return: 部门列表信息对象
        """
        dept_list_result = await DeptDao.get_dept_list(page_object, data_scope_sql)

        return CamelCaseUtil.transform_result(dept_list_result)

    @classmethod
    async def check_dept_data_scope_services(
        cls, dept_id: int, data_scope_sql: ColumnElement
    ) -> CrudResponseModel:
        """
        校验部门是否有数据权限service
        ...
        :return: 校验结果
        """
        depts = await DeptDao.get_dept_list(DeptModel(deptId=dept_id), data_scope_sql)
        if depts:
            return CrudResponseModel(is_success=True, message='校验通过')
        raise ServiceException(message='没有权限访问部门数据')

    @classmethod
    async def check_dept_name_unique_services(cls, page_object: DeptModel) -> bool:
        """
        校验部门名称是否唯一service
        ...
        dept = await DeptDao.get_dept_detail_by_info(
            DeptModel(deptName=page_object.dept_name, parentId=page_object.parent_id)
        )
    ...
    @classmethod
    @transactional()
    async def add_dept_services(cls, page_object: DeptModel) -> CrudResponseModel:
        """
        新增部门信息service
        ...
        :return: 新增部门校验结果
        """
        if not await cls.check_dept_name_unique_services(page_object):
            raise ServiceException(message=f'新增部门{page_object.dept_name}失败，部门名称已存在')
        parent_info = await DeptDao.get_dept_by_id(page_object.parent_id)
        ...
        await DeptDao.add_dept_dao(page_object)
        return CrudResponseModel(is_success=True, message='新增成功')

    @classmethod
    @transactional()
    async def edit_dept_services(cls, page_object: DeptModel) -> CrudResponseModel:
        """
        编辑部门信息service
        ...
        if (
            ...
            and (await DeptDao.count_normal_children_dept_dao(page_object.dept_id)) > 0
        ):
            raise ServiceException(message=f'修改部门{page_object.dept_name}失败，该部门包含未停用的子部门')
        new_parent_dept = await DeptDao.get_dept_by_id(page_object.parent_id)
        old_dept = await DeptDao.get_dept_by_id(page_object.dept_id)
        if new_parent_dept and old_dept:
            ...
            await cls.update_dept_children(page_object.dept_id, new_ancestors, old_ancestors)
        edit_dept = page_object.model_dump(exclude_unset=True)
        await DeptDao.edit_dept_dao(edit_dept)
        if (
            page_object.status == CommonConstant.DEPT_NORMAL
            and page_object.ancestors
            and page_object.ancestors != 0
        ):
            await cls.update_parent_dept_status_normal(page_object)
        return CrudResponseModel(is_success=True, message='更新成功')

    @classmethod
    @transactional()
    async def delete_dept_services(cls, page_object: DeleteDeptModel) -> CrudResponseModel:
        """
        删除部门信息service
        ...
        if page_object.dept_ids:
            ...
            for dept_id in dept_id_list:
                if (await DeptDao.count_children_dept_dao(int(dept_id))) > 0:
                    raise ServiceWarning(message='存在下级部门,不允许删除')
                if (await DeptDao.count_dept_user_dao(int(dept_id))) > 0:
    ...
    async def dept_detail_services(cls, dept_id: int) -> DeptModel:
        ...
        :return: 部门id对应的信息
        """
        dept = await DeptDao.get_dept_detail_by_id(dept_id=dept_id)
    ...
    @classmethod
    async def update_parent_dept_status_normal(cls, dept: DeptModel) -> None:
        """
        更新父部门状态为正常
        ...
        await DeptDao.update_dept_status_normal_dao(list(map(int, dept_id_list)))

    @classmethod
    async def update_dept_children(
        cls, dept_id: int, new_ancestors: str, old_ancestors: str
    ) -> None:
        """
        更新子部门信息
        ...
        :return:
        """
        children = await DeptDao.get_children_dept_dao(dept_id)
        update_children = []
        ...
        if children:
            await DeptDao.update_dept_children_dao(update_children)
