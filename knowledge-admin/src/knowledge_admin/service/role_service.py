...
class RoleService:
    ...
    @classmethod
    async def get_role_select_option_services(cls) -> list[dict[str, Any]]:
        """
        获取角色列表不分页信息service
    ...
    @classmethod
    async def get_role_dept_tree_services(cls, role_id: int) -> RoleDeptQueryModel:
        """
        ...
        """
        role = await cls.role_detail_services(role_id)
        role_dept_list = await RoleDao.get_role_dept_dao(role)
    ...
    async def get_role_list_services(
        ...
        :return: 角色列表信息对象
        """
        role_list_result = await RoleDao.get_role_list(query_object, data_scope_sql, is_page)
    ...
    async def check_role_data_scope_services(
        cls, role_ids: str, data_scope_sql: ColumnElement
    ) -> None:
        """
        校验角色是否有数据权限service
        ...
        if role_id_list:
            for role_id in role_id_list:
                roles = await RoleDao.get_role_list(
                    RolePageQueryModel(roleId=int(role_id)), data_scope_sql, is_page=False
                )
                if roles:
                    continue
    ...
    @classmethod
    async def check_role_name_unique_services(cls, page_object: RoleModel) -> bool:
        """
        校验角色名称是否唯一service
        ...
        role = await RoleDao.get_role_by_info(RoleModel(roleName=page_object.role_name))
        if role and role.role_id != role_id:
            return CommonConstant.NOT_UNIQUE
        return CommonConstant.UNIQUE

    @classmethod
    async def check_role_key_unique_services(cls, page_object: RoleModel) -> bool:
        """
        校验角色权限字符是否唯一service
        ...
        """
        role_id = -1 if page_object.role_id is None else page_object.role_id
        role = await RoleDao.get_role_by_info(RoleModel(roleKey=page_object.role_key))
        if role and role.role_id != role_id:
            return CommonConstant.NOT_UNIQUE
        return CommonConstant.UNIQUE
    ...
    @classmethod
    @transactional()
    async def add_role_services(cls, page_object: AddRoleModel) -> CrudResponseModel:
        """
        新增角色信息service
        ...
        if not await cls.check_role_name_unique_services(page_object):
            raise ServiceException(message=f'新增角色{page_object.role_name}失败，角色名称已存在')
        if not await cls.check_role_key_unique_services(page_object):
            raise ServiceException(message=f'新增角色{page_object.role_name}失败，角色权限已存在')
        add_result = await RoleDao.add_role_dao(add_role)
        role_id = add_result.role_id
        if page_object.menu_ids:
            for menu in page_object.menu_ids:
                await RoleDao.add_role_menu_dao(RoleMenuModel(roleId=role_id, menuId=menu))
        return CrudResponseModel(is_success=True, message='新增成功')

    @classmethod
    @transactional()
    async def edit_role_services(cls, page_object: AddRoleModel) -> CrudResponseModel:
        """
        编辑角色信息service
        ...
        if page_object.type == 'status':
            del edit_role['type']
        role_info = await cls.role_detail_services(edit_role.get('role_id'))
        if role_info:
            if page_object.type != 'status':
                if not await cls.check_role_name_unique_services(page_object):
            ...
            await RoleDao.edit_role_dao(edit_role)
            if page_object.type != 'status':
                await RoleDao.delete_role_menu_dao(RoleMenuModel(roleId=page_object.role_id))
                if page_object.menu_ids:
                    for menu in page_object.menu_ids:
    ...
    @classmethod
    @transactional()
    async def role_datascope_services(cls, page_object: AddRoleModel) -> CrudResponseModel:
        """
        分配角色数据权限service
        ...
        role_info = await cls.role_detail_services(page_object.role_id)
        if role_info.role_id:
            await RoleDao.edit_role_dao(edit_role)
            await RoleDao.delete_role_dept_dao(RoleDeptModel(roleId=page_object.role_id))
    ...
    @classmethod
    @transactional()
    async def delete_role_services(cls, page_object: DeleteRoleModel) -> CrudResponseModel:
        """
        删除角色信息service
        ...
        if page_object.role_ids:
            ...
            for role_id in role_id_list:
                role = await cls.role_detail_services(int(role_id))
                if (await RoleDao.count_user_role_dao(int(role_id))) > 0:
                ...
                await RoleDao.delete_role_menu_dao(RoleMenuModel(**role_id_dict))
    ...
    async def role_detail_services(cls, role_id: int) -> RoleModel:
        ...
        :return: 角色id对应的信息
        """
        role = await RoleDao.get_role_detail_by_id(role_id=role_id)
    ...
    async def get_role_user_allocated_list_services(
        ...
        query_user_list = await UserDao.get_user_role_allocated_list_by_role_id(
            page_object, data_scope_sql, is_page
        )
