...
from knowledge_admin.mapper.dao.notice_dao import NoticeDao
from knowledge_admin.vo.notice_vo import DeleteNoticeModel, NoticeModel, NoticePageQueryModel
...
class NoticeService:
    ...
    async def get_notice_list_services(
        cls, query_object: NoticePageQueryModel, is_page: bool = True
        ...
        :return: 通知公告列表信息对象
        """
        notice_list_result = await NoticeDao.get_notice_list(query_object, is_page)

        return notice_list_result

    @classmethod
    async def check_notice_unique_services(cls, page_object: NoticeModel) -> bool:
        """
        校验通知公告是否存在service
        ...
        notice = await NoticeDao.get_notice_detail_by_info(page_object)
    ...
    @classmethod
    @transactional()
    async def add_notice_services(cls, page_object: NoticeModel) -> CrudResponseModel:
        """
        新增通知公告信息service
        ...
        :return: 新增通知公告校验结果
        """
        if not await cls.check_notice_unique_services(page_object):
            raise ServiceException(message=f'新增通知公告{page_object.notice_title}失败，通知公告已存在')
        await NoticeDao.add_notice_dao(page_object)
        return CrudResponseModel(is_success=True, message='新增成功')

    @classmethod
    @transactional()
    async def edit_notice_services(cls, page_object: NoticeModel) -> CrudResponseModel:
        """
        编辑通知公告信息service
        ...
        notice_info = await cls.notice_detail_services(page_object.notice_id)
    ...
    @classmethod
    @transactional()
    async def delete_notice_services(cls, page_object: DeleteNoticeModel) -> CrudResponseModel:
        """
        删除通知公告信息service
    ...
    async def notice_detail_services(cls, notice_id: int) -> NoticeModel:
        ...
        :return: 通知公告id对应的信息
        """
        notice = await NoticeDao.get_notice_detail_by_id(notice_id=notice_id)
