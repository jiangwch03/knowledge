...
class PostService:
    ...
    async def get_post_list_services(
        ...
        :return: 岗位列表信息对象
        """
        post_list_result = await PostDao.get_post_list(query_object, is_page)

        return post_list_result

    @classmethod
    async def check_post_name_unique_services(cls, page_object: PostModel) -> bool:
        """
        检查岗位名称是否唯一service
        ...
        post = await PostDao.get_post_detail_by_info(PostModel(postName=page_object.post_name))
        if post and post.post_id != post_id:
            return CommonConstant.NOT_UNIQUE
        return CommonConstant.UNIQUE

    @classmethod
    async def check_post_code_unique_services(cls, page_object: PostModel) -> bool:
        """
        检查岗位编码是否唯一service
        ...
        """
        post_id = -1 if page_object.post_id is None else page_object.post_id
        post = await PostDao.get_post_detail_by_info(PostModel(postCode=page_object.post_code))
        if post and post.post_id != post_id:
            return CommonConstant.NOT_UNIQUE
        return CommonConstant.UNIQUE
    ...
    @classmethod
    @transactional()
    async def add_post_services(cls, page_object: PostModel) -> CrudResponseModel:
        """
        新增岗位信息service
        ...
        :return: 新增岗位校验结果
        """
        if not await cls.check_post_name_unique_services(page_object):
            raise ServiceException(message=f'新增岗位{page_object.post_name}失败，岗位名称已存在')
        if not await cls.check_post_code_unique_services(page_object):
            raise ServiceException(message=f'新增岗位{page_object.post_name}失败，岗位编码已存在')
        await PostDao.add_post_dao(page_object)
        return CrudResponseModel(is_success=True, message='新增成功')

    @classmethod
    @transactional()
    async def edit_post_services(cls, page_object: PostModel) -> CrudResponseModel:
        """
        编辑岗位信息service
        ...
        post_info = await cls.post_detail_services(page_object.post_id)
    ...
    @classmethod
    @transactional()
    async def delete_post_services(cls, page_object: DeletePostModel) -> CrudResponseModel:
        """
        删除岗位信息service
        ...
        if page_object.post_ids:
            ...
            for post_id in post_id_list:
                post = await cls.post_detail_services(int(post_id))
                if (await PostDao.count_user_post_dao(int(post_id))) > 0:
    ...
    async def post_detail_services(cls, post_id: int) -> PostModel:
        ...
        :return: 岗位id对应的信息
        """
        post = await PostDao.get_post_detail_by_id(post_id=post_id)
