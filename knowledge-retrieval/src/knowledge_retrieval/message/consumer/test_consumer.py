"""
knowledge-retrieval 测试消费者示例

订阅 topic=`test:retrieval:demo` 的消息,打印接收内容,验证消息流端到端链路。
- topic:   test:retrieval:demo
- group:   retrieval_test_demo
...
@consumer(topic='test:retrieval:demo', group_id = StreamTopicConfig.group_id)
async def handle_retrieval_test_demo(msg: Message) -> None:
    """
    retrieval 端测试消费者
    ...
    """
    logger.info(
        f'📥 [retrieval-test-consumer] 收到消息: '
