...
class RagBroadcastTestPublisher:
    """
    rag 端广播测试发送类(对应 subscriber/test_subscriber.py)
    ...
    async def send_demo(cls, payload: dict | None = None) -> int | None:
        ...
        try:
            ...
            logger.info(
                f'📤 [rag-broadcast-test] 已发送测试广播: '
                f'channel={cls.CHANNEL} receivers={receivers} payload={value}'
            )
            return receivers
        except BroadcastError as e:
            logger.warning(
                f'⚠️ [rag-broadcast-test] 测试广播发送失败(忽略,不影响启动): {e}'
            )
            return None


__all__ = ['RagBroadcastTestPublisher']
