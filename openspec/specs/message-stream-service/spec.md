# Capability: message-stream-service

## Purpose

在 `knowledge-common` 沉淀一套业务可用的消息流服务，让业务层用声明式 `@consumer` 装饰器声明消费点、用 `produce` 推送消息，并通过 `StreamBackend` 抽象屏蔽 Redis Stream 与 Kafka 的协议差异。框架提供门面 API、抽象后端、双后端实现（Redis Stream / Kafka）、配置驱动的后端切换能力，确保未来在 Redis 与 Kafka 间切换时业务层、装饰器、门面 API、消息结构全部零修改，只需 `.env` 一行即可生效。
