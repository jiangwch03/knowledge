...
- **新增 `knowledge_admin.message.consumer.log_consumer` 与 `knowledge_rag.message.consumer.log_consumer`**:用 `@consumer(topic, group_id)` 装饰器声明 admin / rag 各自的"操作日志"与"登录日志"消费者,落库逻辑迁移自 `LogAggregatorService._process_messages`。
...
- **按 `app_name` 隔离改为按 topic 命名隔离**:延续原"admin 端只产销 `log:stream:knowledge-admin`、rag 端只产销 `log:stream:knowledge-rag`"的契约,新框架下表达为 `topic = log:operation:{app_name}` / `log:login:{app_name}`,消费者用同样的 topic 订阅;`group_id` 仍按 app 隔离,确保跨 app 不串扰。
...
- **修改代码**
  ...
  - `knowledge-rag/src/knowledge_rag/server/server.py`:同 admin。
- **新增代码**
  ...
  - `knowledge-rag/src/knowledge_rag/message/consumer/log_consumer.py`:rag 端同上,topic / group_id 用 rag 的 app_name 后缀。
