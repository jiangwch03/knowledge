...
- 在 `knowledge-content` 后端实现文件上传、总页数解析、版本预占、MinerU 解析调度、TXT 转 Markdown、解析结果合并入库、失败重试与用户决策等全链路能力（参考需求文档 §5 / §6）。
- 在 `knowledge-common` 中下沉 `knowledge_ai_models` 的 DO/DAO/VO，供 `knowledge-admin` 与 `knowledge-content` 共享（参考需求文档 §9.2 / §10.2.1）。
...
- **后端**: `knowledge-content` 新增 Controller/Service/DAO/VO、消息流消费者、定时任务；`knowledge-admin` 新增模型功能适配接口与字典初始化 SQL。
