...
- [x] 3.1 `JobDao.get_job_list_for_scheduler` 增加 `app_scope` 过滤参数，admin 加载 `'knowledge-admin'` + NULL，rag 加载 `'knowledge-rag'`
...
- [x] 6.2 `knowledge-rag/server/server.py` 调用 `init_system_scheduler` 时传入 `'knowledge-rag'`
...
- [x] 8.4 `sql/ruoyi-fastapi.sql` 中插入 `sys_dict_data` 记录：`knowledge-admin`、`knowledge-rag`、`knowledge-agent`
