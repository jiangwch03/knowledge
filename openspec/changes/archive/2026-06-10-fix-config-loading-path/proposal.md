...
本地开发启动时，`knowledge-common` 组件中的 `env.py` 使用 `os.getcwd()` 作为基准路径查找 `.env` 配置文件。由于各子项目（`knowledge-admin`、`knowledge-content`）的配置文件独立存放在各自目录的 `src/configs/` 下，而启动命令的执行目录不确定（可能在 workspace 根目录，也可能在子项目目录），导致经常无法正确加载配置文件，服务以默认配置启动或配置缺失。
...
- **受影响模块**: 所有导入 `knowledge_common.config.env` 的子项目（`knowledge-admin`、`knowledge-content`）
